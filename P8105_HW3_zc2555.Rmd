...
The dataset has 35 obs and have 1443 variables.  There are four date variables : ‘day_id’, ‘week’, ‘day_id’, ‘day’ , ‘weekday_end’(weekday or weekend), and 1440 activities variables which count for each minute of a 24-hour day starting at midnight.
...
From the table,we can see that the among weekdays, monday the activity levels are usually not hight, while wednesday and thursday are higher, on weekends, the activity level usually drops
...
#group_by(day) %>%
# summarise(activity_day_mean = mean (activity_sum)) %>%
#  mutate(day = factor(day, levels = c( 'Monday', 'Tuesday', 'Wednesday', #'Thursday','Friday', 'Saturday','Sunday')))  

#    ggplot(aes(x = day, y = activity_day_mean)) +
#  geom_bar(stat = 'identity') +
#  ggtitle ("Aggregate minites of total activity from Mon-Fri")+
#  ylab ("total minutes of activities" )
